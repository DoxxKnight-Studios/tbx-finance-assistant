...
import { exportResultAsCsv, exportResultAsJson, getExportableRankingRows } from "@/lib/export";
import { formatCurrency, splitAroundAmount } from "@/lib/format";
import { cn } from "@/lib/utils";
import type { ChatApiResult } from "@/types/chat";

function CollapsibleTriggerLabel({ open, children }: { open: boolean; children: string }) {
  return (
    <>
      {children}
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-card/70 px-5 py-4 backdrop-blur-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger aria-label="Export this response" className="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--brand-ring)">
                ...
                <DropdownMenuContent align="end">
                  ...
                  {csvRows && (
                    <DropdownMenuItem onSelect={() => exportResultAsCsv(result)}>
                      Export as CSV
                    </DropdownMenuItem>
                  )}
                </DropdownMenuContent>
          ...
          <div className="mt-3 flex flex-col items-start gap-2">
            {result.evidence && (
              <Collapsible open={evidenceOpen} onOpenChange={setEvidenceOpen} className="w-full min-w-0">
                <CollapsibleTrigger className="inline-flex items-center gap-1 rounded text-xs font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-(--brand-ring) focus-visible:outline-none">
                  <CollapsibleTriggerLabel open={evidenceOpen}>View evidence</CollapsibleTriggerLabel>
                </CollapsibleTrigger>
                <CollapsibleContent>
            ...
            {result.technical && (
              <Collapsible open={traceOpen} onOpenChange={setTraceOpen} className="w-full min-w-0">
                <CollapsibleTrigger className="inline-flex items-center gap-1 rounded text-xs font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-(--brand-ring) focus-visible:outline-none">
                  <CollapsibleTriggerLabel open={traceOpen}>How this answer was derived</CollapsibleTriggerLabel>
                </CollapsibleTrigger>
                <CollapsibleContent>
